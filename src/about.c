...
static GtkWidget *create_dialog(void)
{
	...
	char *s = g_strdup_printf( "<span size=\"larger\" weight=\"bold\">Geany %s</span>\nmodified by overcq &lt;<a href=\"mailto:overcq@int.pl\">overcq@int.pl</a>&gt;", main_get_version_string() );
	...
	s = g_strdup_printf( "<span size=\"larger\" weight=\"bold\">%s</span>", _( "A fast and lightweight IDE" ));
	gtk_label_set_markup( GTK_LABEL( label_info ), s );
	g_free(s);
	...
	gtk_label_set_markup( GTK_LABEL(codename_label), "<span weight=\"bold\">\"" GEANY_CODENAME "\"</span>" );
	...
	s = g_strdup_printf( _( "(built on or after %s)" ), build_date );
	g_free(build_date);
	char *s_2 = g_strdup_printf( "<span size=\"smaller\">%s</span>", s );
	g_free(s);
	gtk_label_set_markup( GTK_LABEL( builddate_label ), s_2 );
	g_free( s_2 );
	...
	s = g_strdup_printf( _( "Using GTK+ v%u.%u.%u and GLib v%u.%u.%u runtime libraries" ),
		gtk_major_version, gtk_minor_version, gtk_micro_version,
		glib_major_version, glib_minor_version, glib_micro_version);
	s_2 = g_strdup_printf( "<span size=\"smaller\">%s</span>", s );
	g_free(s);
	gtk_label_set_markup( GTK_LABEL( runtime_label ), s_2 );
	g_free( s_2 );
	...
	gtk_label_set_markup( GTK_LABEL(cop_label), _( "Copyright (c) 2005\nThe Geany contributors" ));
	...
	row = 0;
	s = g_strdup_printf( "<span size=\"larger\" weight=\"bold\">%s</span>", _( "Developers" ));
	...
	s = g_strdup_printf( "Colomban Wendling - %s", _( "maintainer" ));
	ROW( s, row, 0, 0, 0, 1 );
	g_free(s);
	row++;
	...
	s = g_strdup_printf( "Nick Treleaven - %s", _( "developer" ));
	ROW( s, row, 0, 0, 0, 1 );
	g_free(s);
	row++;
	...
	s = g_strdup_printf( "Enrico Tröger - %s", _( "developer" ));
	ROW( s, row, 0, 0, 0, 1 );
	g_free(s);
	row++;
	...
	s = g_strdup_printf( "Frank Lanitz - %s", _( "translation maintainer" ));
	ROW( s, row, 0, 0, 0, 1 );
	g_free(s);
	row++;
	...
	s = g_strdup_printf( "<span size=\"larger\" weight=\"bold\">%s</span>", _( "Translators" ));
	label = gtk_label_new(s);
	g_free(s);
	...
	s = g_strdup_printf( "<span size=\"larger\" weight=\"bold\">%s</span>", _( "Previous Translators" ));
	label = gtk_label_new(s);
	g_free(s);
	gtk_table_attach(GTK_TABLE(table), label, 0, 2, row, row + 1,
					(GtkAttachOptions) (GTK_FILL),
	...
	s = g_strdup_printf( "<span size=\"larger\" weight=\"bold\">%s</span>", _( "Contributors" ));
	label = gtk_label_new(s);
	g_free(s);
	gtk_table_attach(GTK_TABLE(table), label, 0, 2, row, row + 1,
					(GtkAttachOptions) (GTK_FILL),
	...
	s = g_strdup_printf( _( "Some of the many contributors (for a more detailed list, see the file %s):" ),
#ifdef G_OS_WIN32
			"Thanks.txt"
#else
			"THANKS"
	...
	s = g_strdup_printf( "%s" G_DIR_SEPARATOR_S "GPL-2", app->datadir );

	g_file_get_contents( s, &license_text, NULL, NULL );
	g_free(s);
